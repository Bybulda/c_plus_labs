...
int main(){
    ...
    all->deallocate(mass);
    all->deallocate(ar);
    array = reinterpret_cast<int*>(all->allocate(sizeof(int) * 8));
    mass = reinterpret_cast<char *>(all->allocate(sizeof(char) * 100));
    all->deallocate(array);
    ar = reinterpret_cast<int*>(all->allocate(sizeof(int) * 100));
